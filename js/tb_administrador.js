    ...
    return new Promise((resolve) => {
        ...
        for (let [id, fila] of Object.entries(data)) {
            ...
            for (let [columna, value] of Object.entries(fila)) {
                ...
                if (columna != "id_admin_fk" && columna != "id_login") {
...
var listar_admin = async () => {
    let myHeader = new Headers();
    let config = {
        headers: myHeader,
        method: "POST"
    };
    myHeader.set("accept", "table_adm");
...
document.querySelector("tbody").addEventListener("click", async (e) => {
    if (!e.target.classList.value.indexOf("edit")) {
        ...
        let json = {
            ...
            pass: input.children[2].innerHTML,
            estado: input.children[3].innerHTML,
        }
        let myHeader = new Headers();
        let config = {
            headers: myHeader,
        ...
        myHeader.set("accept", "update_adm");
        let peticion = await fetch("../php/api.php", config);
        let res = await peticion.json();
        ...
        let myHeader = new Headers();
        myHeader.set("accept", "delete_adm");
        let config = {
            headers: myHeader,
            method: "POST",
            body: JSON.stringify(json)
        ...
        if (res["Datos eliminados"]) {
            Swal.fire({
                position: 'center',
                icon: 'error',
                title: `El administrador ${json.id} ha sido eliminado`,
                showConfirmButton: false,
                timer: 3000
            })
            input.remove();
